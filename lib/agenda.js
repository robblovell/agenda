...
function createJobs(self, createJob, when, names, data, options, cb) {
  var results = [];
  var pending = names.length;
  var errored = false;
  return names.map(function (name, i) {
    return createJob(self, when, name, data, options, function(err, result) {
      if (err) {
        if (!errored) cb(err);
        errored = true;
        return;
...
Agenda.prototype.every = function(interval, names, data, options, cb) {
  ...
  if (typeof names === 'string' || names instanceof String) {
    return createJob(self, interval, names, data, options, cb);
  } else if (Array.isArray(names)) {
    return createJobs(self, createJob, interval, names, data, options, cb);
  }

  function createJob(self, interval, name, data, options, cb) {
...
Agenda.prototype.schedule = function(when, names, data, cb) {
  ...
  if (typeof names === 'string' || names instanceof String) {
    return createJob(self, when, names, data, null, cb);
  } else if (Array.isArray(names)) {
    return createJobs(self, createJob, when, names, data, null, cb);
  }

  function createJob(self, when, name, data, options, cb) {
    var job = self.create(name, data);
    job.schedule(when);
    job.save(cb);
    return job;
...
Agenda.prototype.record = function(names, fail, complete, cb) {
  var self = this;
  if (typeof names === 'string' || names instanceof String) {
    return createJob(self, null, names, null, null, cb);
  } else if (Array.isArray(names)) {
    return createJobs(self, createJob, null, names, data, null, cb);
  }

  function createJob(self, interval, name, data, options, cb) {
    var job = self.create(name, data);
    job.record(fail,complete);
    job.save(cb);
    return job;
  }
};
